...
local commands = {
	commands = {
		...
		{ cmd = 'time' , description = 'Посмотреть время' ,  text = '/me взглянул{sex} на свои часы с гравировкой Milky и посмотрел{sex} время&/time&/do На часах показано время {get_time}.' , arg = '' , enable = true, waiting = '1.500' , bind = "{}"  },
 		...
 		if id == 220 then
 			...
 			if raknetBitStreamReadInt8(bs) == 17 then
				...
				local cmd = (encoded ~= 0) and raknetBitStreamDecodeString(bs, length + encoded) or raknetBitStreamReadString(bs, length)
 				...
 				if cmd:find('Основная статистика') and check_stats then -- /vanme
